  ...
  <header class="flex flex-wrap md:justify-start md:flex-nowrap z-50 w-full py-7">
    <nav
      ...
      <div class="md:col-span-3">
        <!-- Logo -->
        <a class="flex-none rounded-xl text-xl inline-block font-semibold focus:outline-none focus:opacity-80 text-custom-blue"
          href="/" aria-label="TechLibrary">
          <img src="logo.png" class="w-24"></img>TechLibrary
        </a>
        <!-- End Logo -->
      </div>
      ...
      <div class="flex items-center gap-x-2 ms-auto py-1 md:ps-6 md:order-3 md:col-span-3">
        <div v-if="!$page.props.auth.user">
          <a type="button" :href="route('login')"
            class="py-2 px-3 mr-2 inline-flex items-center hover:cursor-pointer gap-x-2 text-sm font-medium rounded-xl border border-gray-200 text-black hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:border-neutral-700 dark:hover:bg-white/10 dark:text-white dark:hover:text-white">
            Sign in
          </a>
          <a type="button" :href="route('register')"
            class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-xl border border-transparent bg-custom-blue text-white hover:bg-custom-blue-dark transition disabled:opacity-50 disabled:pointer-events-none focus:outline-none focus:bg-custom-blue-dark">
            Sign Up
          </a>
        </div>
        <div v-else>
          <a  class="border-2 p-2 rounded-xl  border-custom-blue bg-custom-blue text-white hover:bg-custom-blue-dark focus:cursor-pointer hover:cursor-pointer" :href="route('dashboard')">{{ $page.props.auth.user.name }}</a>
        </div>
        <div class="md:hidden">
          <button type="button"
      ...
      <div id="navbar-collapse-with-animation"
        ...
        <div
          ...
          <div>
            <a class="relative inline-block text-black before:absolute before:bottom-0.5 before:start-0 before:-z-[1] before:w-full before:h-1 dark:text-white"
              href="/" aria-current="page">Home</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 dark:text-white dark:hover:text-neutral-300"
              :href="route('list-of-books')">Books</a>
          </div>
          <div>
            <a class="inline-block text-black hover:text-gray-600 dark:text-white dark:hover:text-neutral-300"
